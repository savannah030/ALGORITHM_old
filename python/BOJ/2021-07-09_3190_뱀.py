import sys
#import heapq heapq는 우선순위 큐잖아 바보야!!!!!!!!
...
snakeX = deque([1]) # 뱀 몸체는 큐로 구현 deque로 구현할 필요없었음!! 
# 디큐는 안에 튜플 못넣지만 일반 리스트는 안에 튜플 넣을 수 있음 단 일반 리스트는 가장'앞'쪽에 삽입/삭제할때 O(N)걸림
# 이코테 ch5 dfs/bfs 4미로탈출 보니까 가능한데?? 왜 내가 썼을 때는 오류난거지?
snakeY = deque([1])
    ...
    for i in range(1,len(timeArr)):   #i=1~len(timeArr)-1
        ...
        for _ in range(time): #3,12,2
            ...
            if board[x_head][y_head]==2: #이동한 칸에 사과가 있다면(꼬리는 움직이지 않음)
                ...
                snakeX.append(x_head)
                snakeY.append(y_head)
                count += 1
                '''
                for j in range(N+2):
                    print(board[j])
                ...
                snakeX.append(x_head)
                snakeY.append(y_head) 
                x_tail = snakeX.popleft()    #꼬리는 움직여야함
                y_tail = snakeY.popleft()    #꼬리는 움직여야함
