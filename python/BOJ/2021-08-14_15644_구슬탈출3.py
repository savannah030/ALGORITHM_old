...
def bfs(rx,ry,bx,by):
    ...
    while queue:
        rx,ry,bx,by,methods = queue.popleft()
        ...
        for idx,(dx,dy) in enumerate([(0,1),(0,-1),(1,0),(-1,0)]): #동서남북
            ...
            if board[rx+dx][ry+dy]==0 or board[bx+dx][by+dy]==0: ########################### 하나라도 갈 수 있으면 ok!!!!
                ...
                if not visited[newrx][newry][newbx][newby]: # 둘다 방문한 적 있어야 이전에 해봤던 시도인듯? 그리고 4차원 배열로 구현해야함
                    ...
                    queue.append((newrx,newry,newbx,newby,methods+method[idx]))
    return -1
...
methods = bfs(rx,ry,bx,by)

if methods==-1: print(methods)
else:
    print(len(methods))
    print(methods)